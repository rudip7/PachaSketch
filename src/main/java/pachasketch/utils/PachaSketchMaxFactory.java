...
public class PachaSketchMaxFactory {
    public static PachaSketchMax buildWithSizeParameters(int[] catColMap, int[] numColMap,
        ...
        MaxSketch[] baseSketches = new MaxSketch[levels];
        for (int i = 0; i < levels; i++) {
            baseSketches[i] = new MaxSketch(width, depth);
        }
        return new PachaSketchMax(levels, catColMap, numColMap, bases, adTree, materialized,
                catIndex, numIndex, regionIndex, baseSketches);
    }

    public static PachaSketchMax buildWithErrorParametersWD(int[] catColMap, int[] numColMap,
                                                            int levels, int[] bases,
                                                            ADTree adTree, MaterializedCombinations materialized,
        ...
        MaxSketch[] baseSketchesSum = new MaxSketch[levels];
        for (int i = 0; i < levels; i++) {
            baseSketchesSum[i] = new MaxSketch(width, depth);
        }
